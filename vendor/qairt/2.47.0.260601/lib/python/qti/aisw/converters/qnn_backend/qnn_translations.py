...
class QnnTranslationBase(BackendTranslationBase):
    ...
    def is_dtype_quantizable(self, dtype):
        '''
        Check whether the dtype is quantizable or already quantized.
        '''
        return dtype in [ir_graph.QNN_DATATYPE_FLOAT_32, ir_graph.QNN_DATATYPE_FLOAT_16] + qnn_quantized_types
...
class QnnInputTranslation(QnnTranslationBase):
    ...
    def add_op_to_backend(self, node, graph, backend, **kwargs):
        ...
        if backend.process_custom_io_in_qnn_backend:
        ...
        if graph.user_custom_io and custom_quant_params is not None and backend.process_custom_io_in_qnn_backend:
...
class QnnAxisAlignedBboxTransformTranslation(QnnTranslationBase):
    ...
    def get_output_tensor_info(self, backend, node, graph):
        ...
        for output_name in node.output_names:
            tensor_data_type = graph.get_buffer(output_name).dtype

            check_encodings = self.is_dtype_quantizable(tensor_data_type)
...
class QnnCastTranslation(QnnTranslationBase):
    ...
    def add_op_to_backend(self, node, graph, backend, **kwargs):
        ...
        """
        QNN Data Type Downcast Logic - Conversion Matrix

        Float Types Conversion Matrix:
        +---------------------------+----------+----------+----------+
        | Backend Configuration     | FP32     | FP16     | BF16     |
        +---------------------------+----------+----------+----------+
        | bitwidth=16               | FP16     | Do       | FP16     |
        | isBfloat16=False          |          | Nothing  |          |
        +---------------------------+----------+----------+----------+
        | bitwidth=16               | BF16     | BF16     | DO       |
        | isBfloat16=True           |          |          | Nothing  |
        +---------------------------+----------+----------+----------+
        | bitwidth=32               | Do       | FP32     | FP32     |
        |                           | Nothing  |          |          |
        +---------------------------+----------+----------+----------+

        Conversion Rules:
        1. Float conversion priority:
        - bitwidth=32: Always convert to FP32
        - bitwidth=16 + isBfloat16=True: Always convert to BF16
        - bitwidth=16 : Convert to FP16
        """

        if output_type == ir_graph.QNN_DATATYPE_FLOAT_32 and backend.float_bitwidth == 16 \
            and not backend.is_online_construction and not backend.isBfloat16:
            downcast_type = ir_graph.QNN_DATATYPE_FLOAT_16
        elif output_type == ir_graph.QNN_DATATYPE_BFLOAT_16 and backend.float_bitwidth == 16 \
            and not backend.isBfloat16:
            downcast_type = ir_graph.QNN_DATATYPE_FLOAT_16
        elif output_type == ir_graph.QNN_DATATYPE_FLOAT_32 and backend.float_bitwidth == 16 \
            and backend.isBfloat16:
            downcast_type = ir_graph.QNN_DATATYPE_BFLOAT_16
        elif output_type == ir_graph.QNN_DATATYPE_FLOAT_16 and backend.float_bitwidth == 16 \
            and backend.isBfloat16:
            downcast_type = ir_graph.QNN_DATATYPE_BFLOAT_16
        ...
        elif output_type == ir_graph.QNN_DATATYPE_BFLOAT_16 and backend.float_bitwidth == 32:
            downcast_type = ir_graph.QNN_DATATYPE_FLOAT_32
    ...
    def add_op_to_backend(self, node, graph, backend, **kwargs):
        ...
        if (node.op.constant_tensor.is_lazy_data()):
            ...
            backend.add_lazy_tensor(
                ...
                is_bias=is_bias,
                transform_manager=node.op.constant_tensor.transform_manager
            )
        else:
            backend.add_tensor(
                node.op.name,
...
class QnnConvertTranslation(QnnTranslationBase):
    ...
    def add_op_to_backend(self, node, graph, backend, **kwargs):
        ...
        outputs_info = backend.get_outputs_info(node, graph, output_buf.dtype)
    ...
    def get_conv_params(self, backend, graph, node):
        ...
        if node.op.type == op_adapter.Conv2dOp.TRANSLATION_KEY:
            ...
            scalar_params = {ir_graph.QNN_OP_CONV_2D_PARAM_GROUP:
                             ...
                             ir_graph.QNN_OP_CONV_2D_PARAM_REUSE_SPARSE_INDICES:
                                 (numpy_dtype_to_qnn[np.dtype('bool')], np.bool_(node.op.reuse_sparse_indices))
                             }
...
class QnnCreateSparseTranslation(QnnTranslationBase):
    ...
    def add_op_to_backend(self, node, graph, backend, **kwargs):
        output_type = graph.get_buffer(node.output_names[0]).dtype
        check_encodings = True
        if output_type != ir_graph.QNN_DATATYPE_FLOAT_32 and output_type not in qnn_quantized_types:
            check_encodings = False
...
class QnnCropAndResizeTranslation(QnnTranslationBase):
    ...
    def add_op_to_backend(self, node, graph, backend, **kwargs):
        ...
        resize_dims_tensor_info = backend.create_tensor_info(resize_dims_tensor_name,
                                                             ...
                                                             data=resize_dims)
        output_tensor_data_type = graph.get_buffer(node.output_names[0]).dtype
...
class QnnCustomOpTranslation(QnnTranslationBase):
    ...
    def add_op_to_backend(self, node, graph, backend, **kwargs):
        ...
        if node.op.converter_op_package_lib:
            ...
            output_datatypes = node.op.infer_data_type(input_names, input_shapes, input_datatypes,
                                                       input_axis_formats, len(node.output_names))
        node.op.c_op.post_process_op()
...
class QnnBufferTranslation(QnnTranslationBase):
    ...
    def add_op_to_backend(self, node, graph, backend, **kwargs):
        ...
        buffer_padding = node.op.buffer_padding
        ...
        backend.add_node(node.op.name, ir_graph.QNN_OP_BUFFER,
                         ...
                         scalar_params={ir_graph.QNN_OP_BUFFER_PARAM_BUFFER_SIZE:
                                        ...
                                        ir_graph.QNN_OP_BUFFER_PARAM_BUFFER_PADDING:
                                            (numpy_dtype_to_qnn[np.dtype('uint32')], np.uint32(buffer_padding)),
...
class DetectionOutTranslation(QnnTranslationBase):
    ...
    def get_detection_output_info(self, backend, node, graph):
        ...
        for i, output_name in enumerate(output_names):
            check_encodings = False
            output_buf = graph.get_buffer(output_name)
            tensor_data_type = output_buf.dtype
...
@register
class QnnElementWiseMuxTranslation(QnnTranslationBase):
    TARGET = op_adapter.ElementWiseMuxOp.TRANSLATION_KEY

    def add_op_to_backend(self, node, graph, backend, **kwargs):
        # Infer the correct output datatype from the C++ op
        # ElementWiseMux output should match the data inputs, not the condition
        output_dtypes = node.op.c_op.infer_output_datatypes(1)
        output_dtype = output_dtypes[0] if output_dtypes else None

        backend.add_node(node.op.name, qnn_definitions.QNN_OP_ELEMENT_WISE_MUX,
                         node.input_names,
                         backend.get_outputs_info(node, graph, tensor_data_type=output_dtype),
                         macs=node.op.macs)

@register
class QnnElementwiseMeanTranslation(QnnTranslationBase):
    TARGET = op_adapter.MeanOp.TRANSLATION_KEY

    def add_op_to_backend(self, node, graph, backend, **kwargs):
        tensor_data_type = graph.get_buffer(node.output_names[0]).dtype
        check_encodings = self.is_dtype_quantizable(tensor_data_type)
...
class QnnExpandTranslation(QnnTranslationBase):
    ...
    def add_op_to_backend(self, node, graph, backend, **kwargs):
        ...
        input_tensor_data_type = output_buf.dtype
            ...
            if not name or (not backend.is_online_construction and "_unprovided_input_" in name):
                ...
                if i == 14:
                    backend.add_tensor(node.op.name,
                                       node.input_names[i],
                                       qnn_definitions.QNN_TENSOR_TYPE_NULL,
                                       np.ndarray([0]),
                                       check_encodings=False,
...
@register
class QnnInverseTranslation(QnnTranslationBase):
    TARGET = op_adapter.InverseOp.TRANSLATION_KEY
...
class QnnIsInfTranslation(QnnTranslationBase):
    ...
    def add_op_to_backend(self, node, graph, backend, **kwargs):
        ...
        output_tensor_dtype = graph.get_buffer(node.output_names[0]).dtype
        check_encodings = False
...
@register
class QnnIsNanTranslation(QnnTranslationBase):
    TARGET = op_adapter.IsNanOp.TRANSLATION_KEY

    def add_op_to_backend(self, node, graph, backend, **kwargs):
        output_tensor_dtype = graph.get_buffer(node.output_names[0]).dtype
        check_encodings = False
        backend.add_node(node.op.name, ir_graph.QNN_OP_IS_NAN,
                         node.input_names,
                         backend.get_outputs_info(node, graph, tensor_data_type=output_tensor_dtype,
                                                  check_encodings=check_encodings))
...
@register
class QnnLogitTranslation(QnnTranslationBase):
    TARGET = op_adapter.LogitOp.TRANSLATION_KEY

    def add_op_to_backend(self, node, graph, backend, **kwargs):
        backend.add_node(node.op.name, qnn_definitions.QNN_OP_LOGIT,
                         node.input_names,
                         backend.get_outputs_info(node, graph),
                         macs=node.op.macs)
...
class QnnLstmTranslation(QnnTranslationBase):
    ...
    def add_op_to_backend(self, node, graph, backend, **kwargs):
        ...
        for i, name in enumerate(node.input_names):
            if not name or (not backend.is_online_construction and "_unprovided_input_" in name):
                node.input_names[i] = node.op.name + "_unprovided_input_" + str(i)
                # Input 24 is reset, should be bool. Ensures it is not quantized
                if i == 24:
                    backend.add_tensor(node.op.name,
                                       node.input_names[i],
                                       qnn_definitions.QNN_TENSOR_TYPE_NULL,
    ...
    def add_op_to_backend(self, node, graph, backend, **kwargs):
        ...
        if backend.c_ir_graph is not None and backend.c_ir_graph.get_output_tensor(node.input_names[1]) is not None:
            ...
            if input_bw != 8:
                ...
                convert_op = op_adapter.ConvertOp(convert_name, to_type=ir_graph.QNN_DATATYPE_UFIXED_POINT_8)
...
class QnnCombinedNmsTranslation(QnnTranslationBase):
    ...
    def add_op_to_backend(self, node, graph, backend, **kwargs):
        ...
        for i, output_name in enumerate(output_names):
            ...
            output_buf = graph.get_buffer(output_name)
            tensor_data_type = output_buf.dtype
            if i != 0 and self.is_dtype_quantizable(tensor_data_type):
                # scores
                check_encodings = True
            if i == 2:
                # class labels
...
class QnnQuantizeTranslation(QnnTranslationBase):
    ...
    def add_op_to_backend(self, node, graph, backend, **kwargs):
        output_buf = graph.get_buffer(node.output_names[0])
        outputs_info = backend.get_outputs_info(node, graph, tensor_data_type=output_buf.dtype)
...
@register
class QnnRandomUniformLikeTranslation(QnnTranslationBase):
    TARGET = op_adapter.RandomUniformLikeOp.TRANSLATION_KEY
    ...
    def add_op_to_backend(self, node, graph, backend, **kwargs):
        ...
        tensor_params = {}
        scalar_params = {ir_graph.QNN_OP_RANDOM_UNIFORM_LIKE_PARAM_LOW: (numpy_dtype_to_qnn[np.dtype('float32')], np.float32(low)),
                         ir_graph.QNN_OP_RANDOM_UNIFORM_LIKE_PARAM_HIGH: (numpy_dtype_to_qnn[np.dtype('float32')], np.float32(high)),}

        # seed attribute handling as per QNN OP def.
        # If present QNN OP def expect seed to be passed as in[1].
        if node.op.hasattr(ir_graph.IR_OP_RANDOM_UNIFORM_LIKE_PARAM_SEED):
            seed = node.op.seed
            seed_tensor_name = backend.create_unique_qnn_tensor_name(node.op.name, ir_graph.IR_OP_RANDOM_UNIFORM_LIKE_PARAM_SEED)
            backend.add_tensor(
                node.op.name,
                seed_tensor_name,
                qnn_definitions.QNN_TENSOR_TYPE_STATIC,
                np.asarray([seed], dtype=np.float32),
                check_encodings=False,
                orig_tensor_name = ir_graph.IR_OP_RANDOM_UNIFORM_LIKE_PARAM_SEED
            )
            node.input_names.append(seed_tensor_name)

        # Input in[0] handling as per QNN op def.
        # QNN op def expects in[0] to be a 1D tensor of type uint32 consisting of source input shape info.
        input_buf = graph.get_buffer(input_names[0])
        input_dtype = input_buf.dtype
        shape = input_buf.shape.dims
        new_input = np.array(shape, dtype=np.uint32)
        new_input_name = node.op.name + "_input"
        new_input_constant_op = op_adapter.ConstantOp(new_input_name, tensor=new_input)
        check_encodings = False
        src_axis_format = graph.get_buffer(node.input_names[0]).get_src_axis_format()
        tensor_axis_format = graph.get_buffer(node.input_names[0]).get_axis_format()
        # extract permute order for this tensor
        permute_order_to_src = graph.get_buffer(node.input_names[0]).perm_to_src
        backend.add_tensor(
            new_input_name,
            new_input_name,
            qnn_definitions.QNN_TENSOR_TYPE_STATIC,
            new_input_constant_op.tensor,
            check_encodings=check_encodings,
            tensor_data_type=ir_graph.QNN_DATATYPE_UINT_32,
            src_axis_format=src_axis_format,
            tensor_axis_format=tensor_axis_format,
            permute_order_to_src=permute_order_to_src,
            orig_tensor_name=input_names[0]
        )
        input_names[0] = new_input_name
        self.insert_trace_info(new_input_name, (node.op.name, TraceType.OP), graph)

        # TODO support based on dtype attribute
        output_info = backend.get_outputs_info(node, graph, tensor_data_type=input_dtype)
        backend.add_node(node.op.name, ir_graph.QNN_OP_RANDOM_UNIFORM_LIKE,
                         node.input_names,
                         output_info,
                         scalar_params=scalar_params,
                         tensor_params=tensor_params)
...
@register
class QnnRandomNormalLikeTranslation(QnnTranslationBase):
    TARGET = op_adapter.RandomNormalLikeOp.TRANSLATION_KEY

    def add_op_to_backend(self, node, graph, backend, **kwargs):
        input_names = node.input_names
        mean = node.op.mean
        scale = node.op.scale
        np_dtype = np.dtype(node.op.dtype)
        dtype = numpy_dtype_to_qnn[np_dtype]

        tensor_params = {}
        scalar_params = {ir_graph.QNN_OP_RANDOM_NORMAL_LIKE_PARAM_MEAN: (numpy_dtype_to_qnn[np.dtype('float32')], np.float32(mean)),
                         ir_graph.QNN_OP_RANDOM_NORMAL_LIKE_PARAM_SCALE: (numpy_dtype_to_qnn[np.dtype('float32')], np.float32(scale))}

        output_info = backend.get_outputs_info(node, graph, tensor_data_type=dtype)
        backend.add_node(node.op.name, ir_graph.QNN_OP_RANDOM_NORMAL_LIKE,
                         node.input_names,
                         output_info,
                         scalar_params=scalar_params,
                         tensor_params=tensor_params)
...
@register
class QnnRotaryEmbeddingTranslation(QnnTranslationBase):
    TARGET = op_adapter.RotaryEmbeddingOp.TRANSLATION_KEY

    def add_op_to_backend(self, node, graph, backend, **kwargs):
        input_names = node.input_names
        interleaved = node.op.interleaved
        rotary_embedding_dim = getattr(node.op, 'rotary_embedding_dim', None)

        # Convert interleaved to bool
        if isinstance(interleaved, np.ndarray):
            interleaved_value = bool(interleaved.item() if interleaved.size == 1 else interleaved.flatten()[0])
        elif isinstance(interleaved, (int, np.integer, np.bool_)):
            interleaved_value = bool(interleaved)
        else:
            interleaved_value = bool(interleaved)

        tensor_params = {}
        scalar_params = {
            ir_graph.QNN_OP_ROTARY_EMBEDDING_PARAM_INTERLEAVED:
                (numpy_dtype_to_qnn[np.dtype('bool')], interleaved_value)
        }

        if rotary_embedding_dim:
            scalar_params[ir_graph.QNN_OP_ROTARY_EMBEDDING_PARAM_ROTARY_EMBEDDING_DIM] = \
                (numpy_dtype_to_qnn[np.dtype('uint32')], np.uint32(rotary_embedding_dim))

        input_buf = graph.get_buffer(input_names[0])
        input_dtype = input_buf.dtype
        output_info = backend.get_outputs_info(node, graph, tensor_data_type=input_dtype)

        backend.add_node(node.op.name, ir_graph.QNN_OP_ROTARY_EMBEDDING,
                         node.input_names,
                         output_info,
                         scalar_params=scalar_params,
                         tensor_params=tensor_params)
...
class QnnResizeTranslation(QnnTranslationBase):
    ...
    def add_op_to_backend(self, node, graph, backend, **kwargs):
        ...
        if len(output_shape) in [3,5] or node.op.interpolation_mode == ir_graph.QNN_OP_RESIZE_INTERPOLATION_MODE_CUBIC or \
            backend_name == "LPAI":
...
@register
class QnnSTFTTranslation(QnnTranslationBase):
    TARGET = op_adapter.STFTOp.TRANSLATION_KEY

    def add_op_to_backend(self, node, graph, backend, **kwargs):
        # Retrieve attributes from the IR node
        onesided = node.op.onesided
        frame_step = node.op.frame_step
        frame_length = node.op.frame_length

        # Define scalar parameters
        scalar_params = {
            ir_graph.QNN_OP_STFT_PARAM_ONESIDED:
                (numpy_dtype_to_qnn[np.dtype('bool')], np.bool_(onesided)),
            ir_graph.QNN_OP_STFT_PARAM_FRAME_STEP:
                (numpy_dtype_to_qnn[np.dtype('uint32')], np.uint32(frame_step)),
            ir_graph.QNN_OP_STFT_PARAM_FRAME_LENGTH:
                (numpy_dtype_to_qnn[np.dtype('uint32')], np.uint32(frame_length))
        }

        # Add the STFT node to the QNN backend
        backend.add_node(
            node.op.name,
            qnn_definitions.QNN_OP_STFT,
            node.input_names,
            backend.get_outputs_info(node, graph),
            scalar_params=scalar_params,
            macs=node.op.macs
