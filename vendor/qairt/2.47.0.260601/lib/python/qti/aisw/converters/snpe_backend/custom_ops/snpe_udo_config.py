...
class UdoGenerator(QnnOpPackageGenerator):
    ...
    def create_package_info(self, op_package_collection, output_path, config_path, op_package_plugin_collection=None):
        from qti.aisw.converters.snpe_backend.custom_ops.helpers.udo_module_helpers import get_internal_core_types
        if op_package_plugin_collection is not None and not isinstance(op_package_plugin_collection, dict):
            raise TypeError("op_package_plugin_collection must be a dict or None, got: {}".format(type(op_package_plugin_collection).__name__))
        for package_name, package in op_package_collection.items():
            for backend, operators in package.items():
                # Default use_qhpi to False if package_name is not found in op_package_plugin_collection
                use_qhpi = op_package_plugin_collection.get(package_name, False) if op_package_plugin_collection is not None else False

                package_info = QnnPackageInfo(package_name, output_path, backend, operators, use_qhpi=use_qhpi)
